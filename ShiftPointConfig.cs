...
public class ShiftPointConfig
{
    // Maps gear number (1-8) to the RPM where you should upshift
    public Dictionary<int, int> GearRPMThresholds { get; set; } = new();

    // Maximum gear for this vehicle (GT3 = 6, Road cars = 6-7, Formula = 6)
    public int MaxGear { get; set; } = 6;
    ...
    public Dictionary<int, Dictionary<int, float>> AccelerationCurves { get; set; } = new(); // Gear -> (RPM -> accel)

    // Creates a default config with reasonable RPM values
    public static ShiftPointConfig CreateDefault()
    {
        return new ShiftPointConfig
        {
    ...
    public static ShiftPointConfig FromOptimalConfig(OptimalShiftConfig optimalConfig)
    {
        var config = new ShiftPointConfig
        {
            ...
            GearRatios = new Dictionary<int, float>(optimalConfig.GearRatios),
            AccelerationCurves = new Dictionary<int, Dictionary<int, float>>(optimalConfig.AccelerationCurves)
        };
        return config;
    }
